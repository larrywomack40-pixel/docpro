...
(function injectPreviewCSS() {
  ...
  s.textContent = `
    ...
    .template-card.locked { cursor: pointer; opacity: 0.85; }
    .template-card.locked:hover { opacity: 1; }
    ...
    .upgrade-modal-box h3 { margin: 0 0 8px; font-size: 1.4rem; color: #2d2926; }
    .upgrade-modal-box p { color: #666; margin: 0 0 20px; font-size: 0.95rem; }
    .upgrade-modal-box .upgrade-btn {
      display: inline-block; background: linear-gradient(135deg, #c99532, #a67a28);
      color: #fff; padding: 12px 32px; border-radius: 8px; font-weight: 700;
      text-decoration: none; border: none; cursor: pointer; font-size: 1rem; margin-bottom: 10px;
    }
    .upgrade-modal-box .upgrade-btn:hover { transform: scale(1.03); }
    .upgrade-modal-box .cancel-link { display: block; color: #999; cursor: pointer; font-size: 0.85rem; margin-top: 8px; }
  `;
...
// HELPERS
// ============================================
function _tpHashStr(s) {
  var h = 0;
  for (var i = 0; i < s.length; i++) { h = ((h << 5) - h) + s.charCodeAt(i); h |= 0; }
  return Math.abs(h);
}

function _tpDeriveColor(template) {
  var n = (template.name + ' ' + template.style).toLowerCase();
  ...
  var colors = ['#c99532','#2c5aa0','#2d8a4e','#7b2d8e','#d35400','#16a085','#c0392b','#34495e'];
  return colors[_tpHashStr(template.name) % colors.length];
}

// ============================================
// GENERATE PREVIEW HTML
// ============================================
function _tpGeneratePreview(template, category) {
  var color = _tpDeriveColor(template);
  var h = _tpHashStr(template.name);
  ...
  if (cat === 'resumes') {
    ...
    if (layout === 0) {
      return '<div style="text-align:center;margin-bottom:10px"><div style="font-size:14px;font-weight:700">John Doe</div><div style="font-size:8px;color:' + color + '">Software Engineer</div><div style="font-size:7px;color:#999;margin-top:2px">john@email.com | (555) 123-4567</div></div>' +
        ...
        '<div style="font-size:8px;margin-bottom:6px"><div style="font-weight:600">Senior Developer - Tech Corp</div><div style="color:#999;font-size:7px">2022 - Present</div></div>' +
    ...
    if (layout === 1) {
      return '<div style="display:flex;gap:12px"><div style="width:35%;border-right:2px solid ' + color + ';padding-right:10px"><div style="width:35px;height:35px;background:' + color + ';border-radius:50%;margin:0 auto 8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;font-weight:700">JD</div>' +
        '<div style="font-size:7px;font-weight:700;color:' + color + ';margin-bottom:4px">SKILLS</div><div style="font-size:7px;color:#666;line-height:1.6">JavaScript<br>Python<br>React<br>Node.js</div></div>' +
        ...
        '<div style="font-size:7px;font-weight:700;color:' + color + ';margin-bottom:3px">EXPERIENCE</div><div style="font-size:7px;margin-bottom:4px"><strong>Tech Corp</strong> - Senior Dev<br><span style="color:#999">2022 - Present</span></div></div></div>';
    }
  ...
  if (cat === 'contracts') {
    ...
    if (layout === 0) {
      return '<div style="text-align:center;margin-bottom:12px"><div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:2px">Service Agreement</div><div style="width:60px;height:2px;background:' + color + ';margin:6px auto"></div><div style="font-size:7px;color:#999">' + template.name + '</div></div>' +
        '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:8px"><strong>1. Scope of Work</strong><br>The Service Provider agrees to deliver professional services as outlined in Exhibit A...</div>' +
        '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:10px"><strong>2. Compensation</strong><br>Client agrees to pay the amount of $____ for services rendered...</div>' +
    ...
    if (layout === 1) {
      return '<div style="background:' + color + ';color:#fff;padding:10px 15px;margin:-20px -20px 12px;font-size:13px;font-weight:700">CONTRACT</div>' +
        ...
        '<div style="font-size:8px;margin-bottom:6px;padding:6px;background:#f9f9f9;border-left:3px solid ' + color + '"><strong>PARTIES:</strong> This agreement is between Party A and Party B.</div>' +
        '<div style="font-size:8px;color:#444;line-height:1.5"><strong>ARTICLE I</strong><br>Contractor shall provide services in accordance with terms set forth herein...</div>';
    }
    return '<div style="font-size:9px;font-weight:700;color:' + color + ';margin-bottom:2px">ARTICLE I</div><div style="font-size:11px;font-weight:700;margin-bottom:8px">General Terms & Conditions</div>' +
      '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:8px"><strong>Section 1.1</strong> - This Agreement is entered into as of the date last signed below between the parties identified herein...</div>' +
      '<div style="font-size:8px;color:#444;line-height:1.6"><strong>Section 1.2</strong> - The term shall commence on the Effective Date and continue for twelve (12) months...</div>' +
  ...
  if (cat === 'letters') {
    ...
    if (layout === 0) {
      return '<div style="text-align:right;font-size:8px;color:#666;margin-bottom:12px">January 1, 2026</div>' +
        ...
        '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:8px">I am writing to express my interest in the position advertised. With over five years of experience, I am confident in my ability to contribute...</div>' +
    ...
    if (layout === 1) {
      return '<div style="border-bottom:2px solid ' + color + ';padding-bottom:8px;margin-bottom:10px"><div style="font-size:13px;font-weight:700;color:' + color + '">Jane Smith</div><div style="font-size:7px;color:#999">123 Main St | City, ST 12345 | jane@email.com</div></div>' +
        ...
        '<div style="font-size:8px;line-height:1.6;color:#444">Dear Sir/Madam,<br><br>I am pleased to submit this letter regarding the open position...</div>';
    }
    return '<div style="background:' + color + ';color:#fff;padding:8px 12px;margin:-20px -20px 12px"><div style="font-size:11px;font-weight:700">FORMAL LETTER</div><div style="font-size:7px;opacity:0.8">' + template.name + '</div></div>' +
      ...
      '<div style="font-size:8px;color:#444;line-height:1.6">This letter serves as formal notification regarding the matter discussed. Please review the enclosed documents...</div>';
  }
  ...
  if (cat === 'proposals') {
    ...
    if (layout === 0) {
      return '<div style="background:linear-gradient(135deg,' + color + ',' + color + 'cc);color:#fff;padding:15px;margin:-20px -20px 12px;text-align:center"><div style="font-size:14px;font-weight:800">PROJECT PROPOSAL</div><div style="font-size:8px;opacity:0.9;margin-top:2px">' + template.name + '</div></div>' +
        ...
        '<div style="font-size:8px;color:#444;line-height:1.5;margin-bottom:8px">We propose a comprehensive solution to address key business challenges and drive measurable growth...</div>' +
    ...
    return '<div style="font-size:16px;font-weight:800;margin-bottom:2px">Business Proposal</div><div style="font-size:8px;color:' + color + ';margin-bottom:12px">' + template.name + '</div>' +
      '<div style="font-size:8px;padding:6px;border-left:3px solid ' + color + ';background:#f9f9f9;margin-bottom:8px"><strong>Objective:</strong> Deliver a tailored solution that increases efficiency by 40% in Q1.</div>' +
      '<div style="font-size:8px;color:#444;line-height:1.5"><strong>Approach:</strong> Our three-phase methodology ensures seamless implementation...</div>';
  }
  ...
  if (cat === 'reports') {
    ...
    if (layout === 0) {
      return '<div style="font-size:14px;font-weight:800;margin-bottom:2px">Annual Report</div><div style="font-size:8px;color:' + color + ';margin-bottom:12px">' + template.name + '</div>' +
        ...
        '<div style="font-size:8px;color:#444;line-height:1.5">This report provides a comprehensive overview of performance metrics for the fiscal year...</div>';
    }
    return '<div style="background:' + color + ';color:#fff;padding:10px 12px;margin:-20px -20px 12px"><div style="font-size:12px;font-weight:700">QUARTERLY REPORT</div><div style="font-size:7px;opacity:0.8">Q4 2025 | ' + template.name + '</div></div>' +
      ...
      '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:6px">1. Revenue exceeded targets by 12%<br>2. Customer satisfaction improved to 94%<br>3. Costs reduced by $180K</div>';
  }
  ...
  if (gLayout === 0) {
    return '<div style="text-align:center;padding:12px 0;border-bottom:2px solid ' + color + ';margin-bottom:10px"><div style="font-size:13px;font-weight:800;text-transform:uppercase">' + category.replace(/s$/, '') + '</div><div style="font-size:7px;color:' + color + '">' + template.name + '</div></div>' +
      '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:6px">This document sets forth the terms and conditions applicable to the parties involved...</div>' +
      '<div style="font-size:8px;color:#444;line-height:1.6">All provisions shall be binding upon execution by authorized representatives...</div>';
  }
  if (gLayout === 1) {
    return '<div style="background:' + color + ';color:#fff;padding:10px 12px;margin:-20px -20px 12px;font-size:12px;font-weight:700">' + category.replace(/s$/, '').toUpperCase() + '</div>' +
      ...
      '<div style="font-size:8px;color:#444;line-height:1.6;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:4px">Section 1: This section outlines the primary terms governing the relationship...</div>' +
      '<div style="font-size:8px;color:#444;line-height:1.6">Section 2: Additional provisions may be appended with mutual written consent...</div>';
  }
  return '<div style="display:flex"><div style="width:4px;background:' + color + ';margin:-20px 12px -20px -20px"></div><div style="flex:1"><div style="font-size:13px;font-weight:700;margin-bottom:2px">' + category.replace(/s$/, '') + '</div><div style="font-size:7px;color:' + color + ';margin-bottom:10px">' + template.name + '</div>' +
    '<div style="font-size:8px;color:#444;line-height:1.6">This document serves as a formal record of the agreement between the undersigned parties.</div></div></div>';
}
...
// ============================================
var _origRenderTemplateGrid = window.renderTemplateGrid;

window.renderTemplateGrid = function(category) {
  var grid = document.getElementById('templateGrid');
  if (!grid) return;

  // Access TEMPLATES from global lexical scope (const in editor.html)
  var tpl;
  try { tpl = TEMPLATES[category]; } catch(e) { tpl = null; }
  if (!tpl) {
...
// ============================================
// AUTO-INIT: Re-render current tab on load
// ============================================
var _origOpenTemplates = window.openTemplates;
window.openTemplates = function() {
  if (_origOpenTemplates) _origOpenTemplates();
  setTimeout(function() {
    var activeTab = document.querySelector('.template-tab.active');
    if (activeTab) {
      var category = activeTab.textContent.replace(/\s*\(\d+\)/, '');
      renderTemplateGrid(category);
    }
  }, 100);
};

console.log('[DraftMyForms] Template preview system loaded v2');
