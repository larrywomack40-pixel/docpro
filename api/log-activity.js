...
module.exports = async function handler(req, res) {
  ...
  // Normalise body once (Vercel may pass a string)
  var pbody = req.body;
  if (typeof pbody === 'string') { try { pbody = JSON.parse(pbody); } catch (e) { pbody = {}; } }
  if (!pbody || typeof pbody !== 'object') pbody = {};

  // --- link_session: associate a visitor_sessions row with a registered user ---
  try {
    const __action = (req.query && req.query.action) || pbody.action;
  ...
  // ---- BEHAVIORAL EVENT TRACKING (type=event) ----
  if (pbody.type === 'event') {
    try {
      const evName = (pbody.event_name || '').toString().trim();
      if (!evName) return res.status(400).json({ ok: false, error: 'event_name required' });
      let evUserId = null, evEmail = (pbody.email || '') || null;
      try {
        const ah = req.headers.authorization || '';
        const tok = ah.replace('Bearer ', '');
        if (tok) {
          const ac = createClient(SB_URL, process.env.SUPABASE_SERVICE_ROLE_KEY || ANON_KEY, { auth: { persistSession: false } });
          const { data: ud } = await ac.auth.getUser(tok);
      ...
      const { error: evErr } = await ec.from('user_events').insert({
        user_id: evUserId,
        email: evEmail,
        session_id: pbody.session_id || null,
        event_name: evName.slice(0, 64),
        page: (pbody.page || '').slice(0, 256) || null,
        metadata: pbody.metadata && typeof pbody.metadata === 'object' ? pbody.metadata : {}
      });
      if (evErr) return res.status(200).json({ ok: false, error: evErr.message });
      return res.status(200).json({ ok: true });
    } catch (ee) { return res.status(200).json({ ok: false, error: ee.message }); }
  }

  // ---- VISITOR TRACKING (type=visit) ----
  const body = pbody;
  if (body.type === 'visit') {
    try {
      ...
      const { error: upsertErr } = await client.from('visitor_sessions').upsert({
        ...
        timezone: geo.timezone || '', isp: geo.org || '', device_type: device,
        browser: b, os: os, referrer: body.referrer || '',
        utm_source: body.utm_source || '', utm_medium: body.utm_medium || '', utm_campaign: body.utm_campaign || '',
        pages_viewed: [page], updated_at: new Date().toISOString()
